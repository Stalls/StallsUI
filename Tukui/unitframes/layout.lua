...
local playerClassRGB = (CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)[select(2,UnitClass("player"))]
...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		if (unit == "player") then
			panel:SetBackdropBorderColor(playerClassRGB.r, playerClassRGB.g, playerClassRGB.b, 0.8)
		elseif (unit == "target") then
			local targetClassRGB = (CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)[select(2,UnitClass("target"))]
			if(targetClassRGB ~= nil) then
				panel:SetBackdropBordercolor(targetClassRGB.r, targetClassRGB.g, targetClassRGB.b, 0.8)
			end
		end
		self.panel = panel
		...
		TukuiDB.CreateGloss(power)
		
		local powerBG = power:CreateTexture(nil, 'BORDER')
		powerBG:SetAllPoints(power)
		powerBG:SetTexture(normTex)
		powerBG:SetVertexColor(.1, .1, .1, 1)
		powerBG.multiplier = 0.3
		
		power.value = TukuiDB.SetFontString(panel, TukuiCF.media.pixelfont, 8, "MONOCHROMEOUTLINE")
        ...
        portrait.PostUpdate = function(self) self:SetAlpha(0) self:SetAlpha(0.15) end -- thanks Karudon
        portrait:SetPoint("TOPLEFT", health)
		portrait:SetPoint("BOTTOMRIGHT", power)
        table.insert(self.__elements, TukuiDB.HidePortrait)
        self.Portrait = portrait
		TukuiDB.CreateGloss(health)
		...
		if (unit == "player") then
			...
			if (TukuiDB.myclass == "WARLOCK" or TukuiDB.myclass == "PALADIN") then
				...
				for i = 1, 3 do					
					...
					--TukuiDB.CreateOverlay(bars[i], TukuiCF.media.empath3)
					if i == 1 then
			...
			if TukuiDB.myclass == "SHAMAN" and db.totemtimer == true then
				...
				for i = 1, 4 do
					...
					TukuiDB.CreateOverlay(TotemBar[i], TukuiCF.media.empath2)
				end
		...
		if (unit == "target") then			
			...
			Name:SetFont(TukuiCF.media.pixelfont, 8, "MONOCHROMEOUTLINE")
	...
	if (unit == "targettarget") then
		...
		if (totClassRGB ~= nil) then
			panel:SetBackdropBorderColor(totClassRGB.r, totClassRGB.g, totClassRGB.b, 0.8)
		end
		self.panel = panel
		panel:SetScript("OnUpdate", TukuiDB.UpdateTarget)
	...
	if (unit == "pet") then
		-- create panel if higher version
		local petClassRGB = {r = 0.3, g = 0.3, b = 0.3}
		if(select(2,UnitClass("pet")) ~= nil) then
			petClassRGB = (CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)[select(2,UnitClass("pet"))]
		end
		...
		if not TukuiDB.lowversion then
			...
			panel:SetFrameLevel(2)
			panel:SetFrameStrata("MEDIUM")
			panel:SetBackdropBorderColor(petClassRGB.r, petClassRGB.g, petClassRGB.b, 0.8)
			self.panel = panel
		end
...
if TukuiCF["unitframes"].maintank == true then
	...
	tank:SetPoint("BOTTOMLEFT", TukuiInfoRight, "TOPLEFT", -42, 450)
end
...
if TukuiCF["unitframes"].mainassist == true then
	...
	if TukuiCF["unitframes"].maintank == true then
		...
		assist:SetPoint("BOTTOMLEFT", TukuiInfoRight, "TOPLEFT", -42, 450)
	end
end
------------------------------------------------------------------------

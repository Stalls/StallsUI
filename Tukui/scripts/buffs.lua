...
TempEnchant1:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-174), TukuiDB.Scale(-10))
...
for i = 1, 3 do
	...
	TukuiDB.CreateGloss(_G["TempEnchant"..i])
	...
	_G["TempEnchant"..i.."Duration"]:SetPoint("BOTTOM", 0, TukuiDB.Scale(0))
...
local function StyleBuffs(buttonName, index, debuff)
	...
	if icon and not _G[buttonName..index.."Panel"] then
		...
		TukuiDB.CreateGloss(buff)
		duration:ClearAllPoints()
		duration:SetPoint("BOTTOM", 0, TukuiDB.Scale(0))
			...
			elseif ( index == 1 ) then
				local mainhand, _, _, offhand, _, _, hand3 = GetWeaponEnchantInfo()
					...
					elseif ((mainhand and not offhand and not hand3) or (offhand and not mainhand and not hand3) or (hand3 and not mainhand and not offhand)) and not UnitHasVehicleUI("player") then
						...
						buff:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-174), TukuiDB.Scale(-10))
					end
			else
...
local function UpdateDebuffAnchors(buttonName, index)
	...
	if index == 1 then
		debuff:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-174), TukuiDB.Scale(-128))
	else
